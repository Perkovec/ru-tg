...
function signUp() {
  return new Promise((resolve, reject) => {
    ...
    const clientPromise = clientProxy.createClientForUser();
    clientPromise.then(dataCenter => {
      userData.setDataCenter(dataCenter);
      ui.spinner();
      ui.spacer();
      askPhoneNumberToSendCode().then(res => {
        logger.info('Send code to phone number response: %s', res.sendCodeRes.toPrintable());
        // ask the received code
        askAuthCode(res.phoneNumber, res.sendCodeRes.phone_code_hash).then(authCodeRes => {
          function saveUser(signUpRes) {
            try {
              var authKeyBuffer = clientProxy.getAuthKey().encrypt("");
